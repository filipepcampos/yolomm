        ...
        if num_iter < num_warmup:
            ...
            for j, x in enumerate(optimizer.param_groups):
                ...
                x["lr"] = np.interp(
                    ...
                    xi,
                    [
                        cfg.TRAIN.WARMUP_BIASE_LR if j == 2 else 0.0,
...
def train_mm(
    ...
    model.train()
    start = time.time()
    for i, (input, target, paths, shapes, proj) in enumerate(train_loader):
        intermediate = time.time()
        # print('tims:{}'.format(intermediate-start))
        num_iter = i + num_batch * (epoch - 1)
...
def validate(
    ...
    # print(save_dir)
    _, imgsz = [
        check_img_size(x, s=max_stride) for x in config.MODEL.IMAGE_SIZE
    ...
    p, r, f1, mp, mr, map50, map, t_inf, t_nms = (
        0.0,
        0.0,
        0.0,
        0.0,
    ...
    if config.TEST.SAVE_JSON and len(jdict):
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            if is_coco:
                eval.params.imgIds = [
                    int(Path(x).stem) for x in val_loader.dataset.img_files
