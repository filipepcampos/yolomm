...
class KITTIDataset(Dataset):
    def __init__(self, cfg, is_train, inputsize=640, transform=None):
        ...
        if is_train:
            indicator = cfg.DATASET.TRAIN_SET # TODO: Make sure is training / testing in the config file
        else:
            indicator = cfg.DATASET.TEST_SET
        self.img_root = img_root / indicator / "image_2"
        self.label_root = label_root / indicator / "label_2"
        self.mask_root = mask_root / indicator / "image_2" # TODO: png files
        # self.lane_root = lane_root / indicator # TODO: Figure out how this works in KITTI?
    ...
    def _get_db(self):
        ...
        for mask in tqdm(list(self.mask_list)):
            mask_path = str(mask)
            print(mask_path)
            # label_path = mask_path.replace(
            #     str(self.mask_root), str(self.label_root)
            # ).replace(".png", ".json")
            # image_path = mask_path.replace(
            #     str(self.mask_root), str(self.img_root)
            # ).replace(".png", ".jpg")
            ...
            # rec = [
            #     {"image": image_path, "label": gt, "mask": mask_path, "lane": lane_path}
            # ]
